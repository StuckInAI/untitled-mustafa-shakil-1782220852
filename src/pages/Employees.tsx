...
import { employees, leaveRequests, performanceReviews, enrollments, courses, getEmployeeName } from '@/lib/mockData';
import { formatCurrency, formatDate } from '@/lib/utils';
import type { Employee, Department } from '@/types';
import {
  Plus, Filter, Download, Mail, Phone, MapPin,
  Building2, Calendar, DollarSign, Users,
  TrendingUp, GraduationCap, FileText, ChevronRight,
  Briefcase, Clock, MoreVertical, Edit3, UserX
} from 'lucide-react';
    ...
    <PageLayout
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" icon={<Plus className="w-3.5 h-3.5" />} onClick={() => setShowAddModal(true)}>Add Employee</Button>
        </div>
      }
    >
      {/* Filters */}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {filtered.map(emp => (
              <tr key={emp.id} className="hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => setSelected(emp)}>
      ...
      {/* Employee Drawer */}
      <Drawer open={!!selected} onClose={() => setSelected(null)} title={selected ? `${selected.firstName} ${selected.lastName}` : ''} width="w-[560px]">
        {selected && (
          <div>
            {/* Header Card */}
            <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-indigo-50">
              <div className="flex items-start gap-4">
                ...
                <div className="flex gap-1">
                  <button className="p-2 rounded-lg hover:bg-white hover:shadow-sm text-slate-500 transition-all">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-600">
                  ...
                  {formatCurrency(selected.salary)} / year
                </div>
              </div>
            </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{employees.find(e => e.id === selected.managerId)?.jobTitle}</p>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={() => setShowAddModal(false)}>Save Employee</Button>
          </>
        }
      >
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            { label: 'Last Name', placeholder: 'Mitchell' },
            { label: 'Email', placeholder: 'sarah@company.com' },
            ...
            <div key={f.label}>
              ...
              <input placeholder={f.placeholder} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-slate-300" />
            </div>
          ))}
          <div>
          ...
          <div>
            ...
            <input type="number" placeholder="85000" className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-slate-300" />
          </div>
          <div>
